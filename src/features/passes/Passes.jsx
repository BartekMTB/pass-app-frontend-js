...
import css from './Passes.module.css'
export const Passes = () => {
  ...
  return (
    <div>
      <ul className={css.passesList}>
        {content.map(({_id,passNumber,personOnPassCompany,baseCreatingPass})=><li className={css.passesItem} key={_id}><p>{passNumber}, {personOnPassCompany}, {baseCreatingPass}</p><button className={css.passBtn}>edit</button></li>)}
      </ul>
    </div>
  )
}